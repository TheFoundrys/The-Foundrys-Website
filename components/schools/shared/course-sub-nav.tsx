...
export function CourseSubNav() {
    ...
    return (
        <>
            {isSticky && <div className="h-[60px] md:h-[76px] w-full" />}
            <div
                className={`w-full transition-all duration-300 ${isSticky ? "fixed bg-white/95 backdrop-blur-md shadow-sm border-b border-slate-200/50 z-[40]" : "relative bg-white border-b border-slate-200 pt-8 pb-4 z-40"}`}
                style={{
                    top: isSticky ? (isNavbarVisible ? '96px' : '0px') : 'auto',
                }}
            >
