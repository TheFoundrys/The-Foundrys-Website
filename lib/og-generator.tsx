...
async function loadGoogleFont(font: string, text: string): Promise<ArrayBuffer | null> {
  try {
    ...
    console.warn("Failed to load font for OG image", e);
    return null;
  }
}
...
export async function GenerateOgImage({ title, category = "The Foundry's", author = "The Foundry's Team", slug }: OgImageProps) {
  // Load Font (Ubuntu Bold)
  const fontData = await loadGoogleFont('Ubuntu:wght@700', title + category + author + "The Foundry's");
